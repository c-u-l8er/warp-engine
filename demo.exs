...
# This script demonstrates the working IsLab Database implementation
...
demo_data_dir = "/tmp/islab_db_demo_#{:os.system_time(:millisecond)}"
Application.put_env(:islab_db, :data_root, demo_data_dir)

IO.puts "🚀 Starting IsLab Database universe at: #{demo_data_dir}"
...
{:ok, :stored, shard3, time3} = IsLabDB.cosmic_put("archive:old_data",
...
{:ok, alice_data, alice_shard, alice_time} = IsLabDB.cosmic_get("user:alice")
{:ok, bob_data, bob_shard, bob_time} = IsLabDB.cosmic_get("user:bob")
...
{:ok, deletion_results, delete_time} = IsLabDB.cosmic_delete("user:bob")
...
case IsLabDB.cosmic_get("user:bob") do
